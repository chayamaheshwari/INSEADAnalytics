...
We observe high correlations between certain questions like 0.64 between Q. 13 & Q. 14,  0.58  between Q.9 & Q.3 or 0.62 between Q 27 & Q. 28. Yes, they make sense. For example Q. 27 talks about boating being number one thing I do in my spare time & Q. 28 talks about boating being my true passion in life. Answers to both the questions should be similar.  Another example is Q. 13 & Q.14 both talk about buying largest and greatest boats & accesories.

<br>

These correlations imply that many of these questions are similar & are looking for the same answer from the participants of the survey but the from different perspectives. Hence, some of these questions can be grouped into one component, reducing the number of questions but at the same time keeping a high level of information. It also implies that these correlations are by design.
...
The table provides the eigen values of the different components. The eigen value is an indication of how many factors/questions each component is able to explain. A general rule is to have atleast those factors whose eigen value is greater than one indicating explaining those many factors. The table also provides the percentage of variance explained & the cumulative variance.  The plot has eigen value against the component numbers. The leg of the graph gives an immediate visual indication of how mnay components to choose for analysis. 

<br>

The plotted line cannot be increasing. It has this shape as it we plot the components based on their decreasing eigen values. It cannot be increasing, it can be decreasing or flat. It shows as we increase no. of components the increase in variance explained reduces. 

<br>

We would like to see fewer components explaining high variance or components with high eigen values. 
...
Component 1 is on reputation & status. Component 2 stresses on a hobby (like adventure, nature etc.) and means of socialization. Component 3 can reflect brand sensitivity of the consumers. Component 4 knowledgeability of boats. Component 5 refers to price senstivity. Yes they make business sense, as these are some of the important consumer characteristics which determine their purchasing habits. 

<br> 


Statistical justification will say 5 components as the first 5 components have eigen value greater than 1. Based on both interpretation argument all 5 components should be chosen and maybe even more if it makes logical sense that the component reflects a consumer behavior. As per business arguments it would be better to have enough components to explain atleast 50% variance. However, the lesser the no. of components explaining this the better as it reduces complexity and maybe even cost.  

<br>

This interpretation as explained in question 1 is 
Reputation 
Hobby 
Brand sensitivity 
Knowleadgibility 
...
Data science can help reduce complexity and help make simple stories from tons of incomprehensible data otherwise. 
...
For instance observor 1 would be very high on brand sensitivity & would consider the boat as a status symbol. He/she would be very low on price sensitivity. In contrast observor 10 is very price sensitive and very low on brand sensitivity. 
...
# Please use numbers, not column names, e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
segmentation_attributes_used = c(10,19,5,12,3)  #c(28,25,27,14,20,8,3,12,13,5,9,11,2,30,24) #c(10,19,5,12,3) 
...
* The numbers in the last segment indicate how different are the segments from the avergae of the population on each of the 29 questions individually. For example, consider question Q1.11, segment 3 at 0.23 is much higher than the average of the population whereas segment 1 with -0.41 is much lower than the average of the population. Hence, segment 3 more than often performs repairs & maintenance on the baot themselves whereas segment 1 does not. Hence this could be a scale on the DIY characteristic of the customer. 

* Segment 1 is mainly female, dont boat alone, do not believe in DIY repairs & maintenance  Would not recommend brand to other boaters. 

* Segment 2 would be people of higher age, which do not consider having a boat as a status symbol but what matters is the price.  

* Segment 3 is of people which consider boat to be a status symbol. They are not too concerned about the price. 

* Segment 4 is not brand sensitive but price sensitive. They will not recommend the brand to others. 

* Segment 5 is passionate about boating, is price sensitive rather than brand sensitive, consider boat as a status symbol. They are of a lower age group & while buying a new boat will not buy the same brand again
...
* Considering statistical arguments and looking at the dendogram & distances, I would choose 8 segments uptil where the line on the distances graph flattens out. Based on interpretation I would choose the 5 segments as each has a distinct quality & different products & different ways of marketing needs to be done to the population in each. Based on business arguments I would probably have fewer segments, maybe 3 to be able to better focus. 

* Segment 1 would be DIY, Segment 2 is the price sensitive one (Economy segment), Segment 3 would be the ones concerned about reputation & status (luxury & Premium segment), Segment 4 are dissatisified consumers & Segment 5 are the passionate boaters. 


* The variance explained would be higer, hence probably a few more smaller segments could be observed. However if the model is robust, the type of segments should converge to the same. 

* I would recommend to have different offerins for Segment 2 & Segment 3 and concentrate on them. Have high quality for low price and market it to consumers of segment 2. Also build a reputable luxury or premium brand for customers of segment 3 and charge it to them at a higher price hence improving profit margins. 

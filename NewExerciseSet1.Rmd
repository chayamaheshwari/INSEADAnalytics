...
---
title: "Exercise Set 1 - Solutions"
...
<br>
A useful R package is FinCal for quick calculation of time value of money, time series analysis & computational finance. Quick tool to caluclate very important ratios to assess financial situation of companies, like, cash ratio, current ratio, quick ratio, Depreciation expenses etc. Also used to calculate discount ratios, future values, yeild of bonds etc. 
<br>
<br>
...
<br>
In dataSet1.R file the function mytcikers is used for "SPY" and "AAPL". This is where the command is given to download the returns of apple during the same period for which returns of S&P500 are downloaded. 
<br>
The cumulative returns of the Apple during this period is `r round(100*sum(StockReturns[,2]),1)`%.
<br>
The average daily returns of the S&P index during this period is `r round(100*mean(StockReturns[,2]),3)`%;
<br>
The standard deviation of the daily returns of the S&P index during this period is `r round(100*sd(StockReturns[,2]),3)`%;
<br>
Yes, we can add a third column to the data in the function myticker for this company (YHOO). Then we can use the function for calculating cumulative, daily returns & standard deviation of the daility returns with [,3] which refers to the third column of the data in the myticker function. 
<br>
<br>
<br>
...
**Your Answers here:**
<br>
The R commands are nrow() & ncol() with the matrix name within the brackets. 
The no. of rows & columnsare found by the following functions. And the Apple returns can be plotted as shown below. The graph of the returns is plotted as well.  
```{r eval = FALSE, echo=TRUE}
...
**Your Answers here:**
<br>
startDate needs to be set to 2001-01-01 in the dataSet1 R file. 
<br>
Returns of more companies can be found by adding this in the myticker function. For example, AMZN for Amazon, AAPl for Apple etc. 
<br>
<br>
...
**Your Answers here:**
<br>
It gives an asnwer of 20. This is because myData is a file with dates in one column, and returns of S&P 500 & Apple in the other two columns for 20 days. It gives no. of instances when this is not equal to StockReturns data. 
<br>
This gives an error as these two matrices do not match dimensionally. 
<br>
<br>
...
<br>
I downloaded a file of data from movies, like director name, actor name, box office collections, facebook page likes on actor's page etc. 
<br>
```{r eval = TRUE, echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
myMovies <- read.csv(file = "movie_metadata.csv", header = TRUE, sep=";")
class(myMovies)
dim(myMovies)
nrow(myMovies)
ncol(myMovies)
